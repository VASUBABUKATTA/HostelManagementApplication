...
public class HostlerDto {
    ...
    @NotBlank(message = "Hostler Alternate mobile number cannot be blank")
    @Pattern(regexp = "(^[6-9][0-9]{9}$)", message = "Hostler mobile number must be 10 digits")
...
//	}
	...
	public HostlerDto(long hostlerId,
			...
			@NotBlank(message = "Hostler Alternate mobile number cannot be blank") @Pattern(regexp = "(^[6-9][0-9]{9}$)", message = "Hostler mobile number must be 10 digits") String hostlerAltMobile,
			...
			@NotBlank(message = "Hostler email is required") @Email(message = "Hostler email must be a valid email address") String hostlerEmail,
			@NotBlank(message = "Hostler's father name is required") @Size(min = 2, max = 50, message = "Father's name must be between 2 and 50 characters") String hostlerFatherName,
			@NotBlank(message = "Hostler address is required") @Size(min = 5, max = 200, message = "Hostler address must be between 5 and 200 characters") String hostlerAddress,
			@NotBlank(message = "Hostler college name is required") @Size(min = 2, max = 100, message = "College name must be between 2 and 100 characters") String hostlerCollegeName,
			@NotBlank(message = "Hostler room number is required") String hostlerRoomNo, Hostel hostel) {
		super();
		this.hostlerId = hostlerId;
		this.hostlerMobile = hostlerMobile;
		this.hostlerAltMobile = hostlerAltMobile;
